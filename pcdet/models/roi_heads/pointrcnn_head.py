...
from .point_mm_roi_head_template import POINTMMRoIHeadTemplate


class PointRCNNHead(POINTMMRoIHeadTemplate):
    ...
    def roipool3d_gpu(self, batch_dict):
        ...
        #
        batch_idx = batch_dict['point_mm_coords'][:, 0]
        #
        point_coords = batch_dict['point_mm_coords'][:, 1:4]
        #
        point_features = batch_dict['point_mm_features']
        #
        rois = batch_dict['mm_rois']  # (B, num_rois, 7 + C)
        ...
        point_scores = batch_dict['point_mm_cls_scores'].detach()
    ...
    def forward(self, batch_dict):
        ...
        targets_dict = self.proposal_layer_mm(
        ...
        if self.training:
            ...
            # mm_rois:(bs,512,7)
            batch_dict['mm_rois'] = targets_dict['mm_rois']
            # mm_roi_labels:(bs,512)
            batch_dict['mm_roi_labels'] = targets_dict['mm_roi_labels']

            batch_dict['mm_roi_scores'] = targets_dict['mm_roi_scores']

        # pooled_features:(bs*160, 216, 133)
        pooled_features = self.roipool3d_gpu(batch_dict)  # (total_rois, num_sampled_points, 3 + C)

        #
        # xyz_input:(1024,5,512,1)
        xyz_input = pooled_features[..., 0:self.num_prefix_channels].transpose(1, 2).unsqueeze(dim=3).contiguous()
        # xyz_features:(1024,128,512,1)
        ...
        # merged_features:(1024,128,512,1)
        ...
        # shared_features:(bs*160,256,1)
        shared_features = l_features[-1]  # (total_rois, num_features, 1)

        # shared_features_mm:(bs*160,256,1)
        batch_dict['shared_features_mm'] = shared_features
        #
        # # (bs*160,1)
        ...
        #         batch_size=batch_dict['batch_size'], rois=batch_dict['mm_rois'], cls_preds=rcnn_cls, box_preds=rcnn_reg
        #     )
        #     batch_dict['mm_batch_cls_preds'] = batch_cls_preds
        #     batch_dict['mm_batch_box_preds'] = batch_box_preds
        #     batch_dict['mm_cls_preds_normalized'] = False
        # else:
        #     targets_dict['mm_rcnn_cls'] = rcnn_cls
        #     targets_dict['mm_rcnn_reg'] = rcnn_reg
        #
