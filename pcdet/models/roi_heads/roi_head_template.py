...
class RoIHeadTemplate(nn.Module):
    ...
    def proposal_layer(self, batch_dict, nms_config):
        ...
        # eg: batch_box_preds:(bs, 37880, 7)
        batch_box_preds = batch_dict['batch_box_preds']
        # eg: batch_cls_preds:(bs, 37880, 1)
        batch_cls_preds = batch_dict['batch_cls_preds']

        # rois:(bs,512,7); 存储经过NMS后的区域提议及其附加特征。
        rois = batch_box_preds.new_zeros((batch_size, nms_config.NMS_POST_MAXSIZE, batch_box_preds.shape[-1]))
        # roi_scores:(bs,512); 存储每个区域提议的得分。
        roi_scores = batch_box_preds.new_zeros((batch_size, nms_config.NMS_POST_MAXSIZE))
        # roi_labels:(bs,512); 存储每个区域提议对应的类别标签
        ...
        for index in range(batch_size):
            ...
            if batch_dict.get('batch_index', None) is not None:
                ...
                batch_mask = index
            # 使用index来筛选当前样本的边界框预测（box_preds）和类别预测（cls_preds）
            # eg: box_preds:(37880, 7);
            box_preds = batch_box_preds[batch_mask]
            # eg: cls_preds:(37880, 1)
            cls_preds = batch_cls_preds[batch_mask]
            #   对于当前样本，计算最大类别概率（cur_roi_scores）及其对应的类别标签（cur_roi_labels）
            ...
            if nms_config.MULTI_CLASSES_NMS:
                ...
                ''' class_agnostic_nms(): 该函数返回经过NMS筛选后的被选中边界框索引（selected）及相应的得分（selected_scores）。'''
                # selected:(512,)
                # selected_scores:(512,)
            ...
            # 将被选中的边界框、得分和类别标签分别填充到先前初始化的rois、roi_scores和roi_labels张量对应位置上（按样本索引index）。
            # rois:(bs,512,7)
        ...
        # 将生成的rois、roi_scores和roi_labels添加到batch_dict中
    ...
    def assign_targets(self, batch_dict, rot_num_id, enable_dif = False):
        ...
        ''' transfer LiDAR coords to local coords: 将激光雷达坐标转换为本地坐标 '''
        gt_of_rois = common_utils.rotate_points_along_z(
            points=gt_of_rois.view(-1, 1, gt_of_rois.shape[-1]), angle=-roi_ry.view(-1)
        ).view(batch_size, -1, gt_of_rois.shape[-1])

        ''' flip orientation if rois have opposite orientation: 如果roi具有相反的方向，则翻转方向 '''
