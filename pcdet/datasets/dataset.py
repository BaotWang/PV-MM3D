...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, is_source=True, root_path=None, logger=None,
        ...
        ''' PointFeatureEncoder(); 创建一个 PointFeatureEncoder对象'''
        ...
        ''' DataAugmentor(); 创建一个 DataAugmentor对象 '''
        self.data_augmentor = DataAugmentor(
            self.root_path, self.dataset_cfg.DATA_AUGMENTOR, self.class_names, logger=self.logger,
        ) if self.training else None

        ''' DataProcessor(); 创建一个 DataProcessor对象 '''
        ...
        for i in range(self.rot_num):
            ...
            if 'mm' in data_dict:
                ...
                if self.dataset_cfg.get('LATER_FUSION', True):
                    ...
                    #
                    if self.training:
                        ''' input_point_discard(); 将点云分成若干个部分，对每个部门进行不同的处理；这里是对近点进行过滤 '''
                        #
                        points_mm2 = self.input_point_discard(points_mm, rate=self.input_discard_rate)
                    else:
                        points_mm2 = self.input_point_discard(points_mm, bin_num=10, rate=self.input_discard_rate)
                    ...
                    # 判断是否 单独使用虚拟点云
                    if self.dataset_cfg.get('POINTS_MM_USE_ALONE', False):
                        #
                        data_dict['points_mm_use_alone' + rot_num_id] = self.dataset_cfg.get('POINTS_MM_USE_ALONE', False)
